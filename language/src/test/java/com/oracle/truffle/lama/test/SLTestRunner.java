...
import com.oracle.truffle.lama.test.SLTestRunner.TestCase;
...
public class SLTestRunner extends ParentRunner<TestCase> {
    ...
    private static final List<NodeFactory<? extends LamaBuiltinNode>> builtins = new ArrayList<>();
    ...
    private static void run(Context context, Path path, PrintWriter out) throws IOException {
        try {
            ...
            Source source = Source.newBuilder(LamaLanguage.ID, path.toFile()).interactive(true).build();
