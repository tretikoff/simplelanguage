...
import static com.oracle.truffle.lama.test.SLExceptionTest.assertGuestFrame;
import static com.oracle.truffle.lama.test.SLExceptionTest.assertHostFrame;
...
public class SLJavaInteropExceptionTest {
    public static class Validator {
        ...
        public void validateNested() throws Exception {
            ...
            try (Context context = Context.newBuilder(LamaLanguage.ID).build()) {
                context.eval(Source.newBuilder(LamaLanguage.ID, sourceText, "Test").build());
                Value test = context.getBindings(LamaLanguage.ID).getMember("test");
    ...
    public void testGuestHostCallbackGuestError() throws Exception {
        ...
        try (Context context = Context.newBuilder(LamaLanguage.ID).build()) {
            context.eval(Source.newBuilder(LamaLanguage.ID, sourceText, "Test").build());
            Value doMultiCallback = context.getBindings(LamaLanguage.ID).getMember("doMultiCallback");
            int numCalbacks = 3;
            try {
